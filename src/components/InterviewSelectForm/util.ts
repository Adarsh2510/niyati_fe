...
export const initialzeInterviewForm = async (
  values: TInterviewFormValues
): Promise<TInterviewRoomResponse> => {
  try {
    console.log('initialzeInterviewForm', JSON.stringify(values, null, 2));
    const response = await fetchWithAuth(
      getNiyatiBackendApiUrl(EBackendEndpoints.INITIALIZE_INTERVIEW),
      {
        method: 'POST',
